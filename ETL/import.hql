...
CREATE TABLE osmnodes AS (SELECT OSMImportNodes(osm_content) FROM osmdata) stored as orc TBLPROPERTIES ('transactional'='true');
CREATE TABLE osmways AS (SELECT OSMImportWays(osm_content) FROM osmdata) stored as orc TBLPROPERTIES ('transactional'='true');
CREATE TABLE osmrelations AS (SELECT OSMImportRelations(osm_content) FROM osmdata) stored as orc TBLPROPERTIES ('transactional'='true');
